...
@Slf4j
@Component
public class TmsCertificationHandler implements BusinessTypeHandler {

    @Override
    public BusinessType getSupportedType() { return BusinessType.TMS_CERTIFICATION; }

    @Override
    public void validate(CreateBusinessRequest request) {
        if (request.getTitle() == null || request.getTitle().isBlank()) {
            throw new BusinessException(400, "资质证书标题不能为空");
        }
    }
    ...
    public String generateBusinessNo(BusinessRecord record) {
        ...
        return BusinessType.TMS_CERTIFICATION.getNoPrefix() + "-" + date + "-" + seq;
    }
}
