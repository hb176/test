...
@Slf4j
@Component
public class QmsDeviationHandler implements BusinessTypeHandler {

    @Override
    public BusinessType getSupportedType() {
        return BusinessType.QMS_DEVIATION;
    }

    @Override
    public void validate(CreateBusinessRequest request) {
        if (request.getTitle() == null || request.getTitle().isBlank()) {
            throw new BusinessException(400, "偏差报告标题不能为空");
        }
    }
    ...
    public String generateBusinessNo(BusinessRecord record) {
        ...
        return BusinessType.QMS_DEVIATION.getNoPrefix() + "-" + date + "-" + seq;
    }

    @Override
    public void afterWorkflowStart(BusinessRecord record, String processInstanceId) {
        log.info("偏差流程已启动: businessNo={}, processInstanceId={}",
