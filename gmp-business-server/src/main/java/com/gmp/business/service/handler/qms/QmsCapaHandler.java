...
@Slf4j
@Component
public class QmsCapaHandler implements BusinessTypeHandler {

    @Override
    public BusinessType getSupportedType() {
        return BusinessType.QMS_CAPA;
    }

    @Override
    public void validate(CreateBusinessRequest request) {
        if (request.getTitle() == null || request.getTitle().isBlank()) {
            throw new BusinessException(400, "CAPA标题不能为空");
        }
    }
    ...
    public String generateBusinessNo(BusinessRecord record) {
        ...
        return BusinessType.QMS_CAPA.getNoPrefix() + "-" + date + "-" + seq;
    }
}
