...
@Slf4j
@Component
public class QmsComplaintHandler implements BusinessTypeHandler {

    @Override
    public BusinessType getSupportedType() { return BusinessType.QMS_COMPLAINT; }

    @Override
    public void validate(CreateBusinessRequest request) {
        if (request.getTitle() == null || request.getTitle().isBlank()) {
            throw new BusinessException(400, "投诉标题不能为空");
        }
    }
    ...
    public String generateBusinessNo(BusinessRecord record) {
        ...
        return BusinessType.QMS_COMPLAINT.getNoPrefix() + "-" + date + "-" + seq;
    }
}
