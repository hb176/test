...
@Slf4j
@Component
public class QmsAuditHandler implements BusinessTypeHandler {

    @Override
    public BusinessType getSupportedType() { return BusinessType.QMS_AUDIT; }

    @Override
    public void validate(CreateBusinessRequest request) {
        if (request.getTitle() == null || request.getTitle().isBlank()) {
            throw new BusinessException(400, "审计计划标题不能为空");
        }
    }
    ...
    public String generateBusinessNo(BusinessRecord record) {
        ...
        return BusinessType.QMS_AUDIT.getNoPrefix() + "-" + date + "-" + seq;
    }
}
