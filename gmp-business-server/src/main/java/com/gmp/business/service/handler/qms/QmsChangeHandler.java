...
@Slf4j
@Component
public class QmsChangeHandler implements BusinessTypeHandler {

    @Override
    public BusinessType getSupportedType() { return BusinessType.QMS_CHANGE; }

    @Override
    public void validate(CreateBusinessRequest request) {
        if (request.getTitle() == null || request.getTitle().isBlank()) {
            throw new BusinessException(400, "变更申请标题不能为空");
        }
    }
    ...
    public String generateBusinessNo(BusinessRecord record) {
        ...
        return BusinessType.QMS_CHANGE.getNoPrefix() + "-" + date + "-" + seq;
    }
}
