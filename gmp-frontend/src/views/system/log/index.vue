  ...
  <el-card>
    <div style="display:flex;justify-content:space-between;margin-bottom:16px">
      <div style="display:flex;gap:12px">
        <el-select v-model="operType" placeholder="操作类型" clearable style="width:120px" @change="fetchData">
        ...
        <el-input v-model="keyword" placeholder="搜索描述" clearable style="width:200px" @keyup.enter="fetchData" />
    ...
    <el-table :data="list" v-loading="loading" border stripe size="small">
      ...
      <el-table-column prop="operType" label="操作类型" width="90" align="center">
        <template #default="{ row }"><el-tag :type="row.operType==='ADD'?'success':row.operType==='DELETE'?'danger':row.operType==='UPDATE'?'warning':'info'" size="small">{{ row.operType }}</el-tag></template>
      </el-table-column>
    ...
    <el-pagination v-model:current-page="pageNum" v-model:page-size="pageSize" :total="total" layout="total,sizes,prev,pager,next" :page-sizes="[10,20,50]" style="margin-top:16px;justify-content:flex-end" @current-change="fetchData" @size-change="pageSize=$event;pageNum=1;fetchData()" />
  </el-card>
</template>
